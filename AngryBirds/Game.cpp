...
Game::Game() : 
	...
	m_gameOver(false)
{
	m_GameWindow.create(sf::VideoMode(m_Width, m_Height), "Angry Birds Clone (c) 2017 Rony Hanna", sf::Style::Default);
...
* @parameter: None
* @return: None
* @Information: Function that begins everything (Initializes the game components and execute the game loop)
********************/
void Game::Run()
{
	InitGameComponents();
...
* @parameter: None
* @return: None
* @Information: Function that initializes the game components (creates boxes, ground, chain, texture, sprites, etc...)
********************/
void Game::InitGameComponents()
{
	...
	// Set text
	m_userInputText.setFont(m_Font);
	m_userInputText.setString(m_Username);
	m_userInputText.setFillColor(sf::Color(44, 127, 255));
	m_userInputText.setStyle(sf::Text::Italic);
...
* @parameter: None
* @return: None
* @Information: Function that checks for user input and renders the current frame
********************/
void Game::GameLoop()
{
...
void Game::Render()
{
	...
	for (b2Body* bodyIter = m_World->GetBodyList(); bodyIter != 0; bodyIter = bodyIter->GetNext())
	{
		...
		if (bodyIter->GetType() == b2_dynamicBody 
			...
			&& bodyIter->GetUserData() != &m_Boxes[0] // Temporary visual fix
